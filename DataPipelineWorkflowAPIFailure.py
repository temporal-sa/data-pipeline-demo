...
with workflow.unsafe.imports_passed_through():
    from activities import extract, validate, transform, load, poll_with_failure
    from dataobjects import DataPipelineParams

@workflow.defn
class DataPipelineWorkflowAPIFailure:
    ...
    async def run(self, input: DataPipelineParams) -> str:
        ...
        activity_output = await workflow.execute_activity(
            poll_with_failure, input, start_to_close_timeout=timedelta(seconds=3000), heartbeat_timeout=timedelta(seconds=20), 
